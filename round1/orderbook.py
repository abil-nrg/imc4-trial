...
class Trader:
    ...
    # Microprice / latency params
    EDGE_THRESHOLD = 0.1
    MICROPRICE_ALPHA = 5.0
    ...
    def compute_imbalance(self, order_depth: OrderDepth):
        ...
        for i in range(min(self.IMBALANCE_LEVELS, len(bids))):
            bid_vol += bids[i][1]

        for i in range(min(self.IMBALANCE_LEVELS, len(asks))):
            ask_vol += -asks[i][1]
    ...
    def latency_aware_quotes(self, best_bid, best_ask, bid_vol, ask_vol, imbalance):
        mid = (best_bid + best_ask) / 2
        mp = self.microprice(best_bid, best_ask, bid_vol, ask_vol)
        edge = mp - mid

        total_edge = edge + 0.5 * imbalance
    ...
    def trade_osmium(self, order_depth: OrderDepth, position: int, history: List[float]):
        ...
        signal = (
            z
            + self.MICROPRICE_ALPHA * edge
            + self.IMBALANCE_ALPHA * imbalance
        )

        limit = self.POSITION_LIMITS["ASH_COATED_OSMIUM"]

        buy_price, sell_price = self.latency_aware_quotes(
            best_bid, best_ask, bid_vol, ask_vol, imbalance
        )
        ...
        if abs(signal) < self.Z_ENTRY:
            if position < limit:
                orders.append(Order("ASH_COATED_OSMIUM", buy_price, self.MM_SIZE))
            if position > -limit:
                orders.append(Order("ASH_COATED_OSMIUM", sell_price, -self.MM_SIZE))
            return orders

        # =====================
        # Strong signal → target position
        # =====================
